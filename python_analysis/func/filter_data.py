...
Functions:
	...
	tweetOnTwitter(df)
	filterDF(df)
	mainFilter(data_files, numb_files)
'''
...
def mainFilter(data_files, numb_files):
	'''
	Main filtering method that will call all helper methods above.

	Parameters:
		data_files: string path to data files - can be changed in config.yml
		numb_files: number of files that you have - used to show progress print statements

	Returns:
		final_df: a filtered pandas dataframe
		may_filtered.csv: saves final_df as a CSV for later use. 
	'''
	numb_files = numb_files
	count = 0.0
	result_dfs = []
	...
	final_df.to_csv('may_filtered.csv')
	return final_df
